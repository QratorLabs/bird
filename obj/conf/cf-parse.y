...
%token FUNCTION PRINT PRINTN UNSET RETURN ACCEPT REJECT QUITBIRD INT BOOL IP PREFIX PAIR QUAD EC SET STRING BGPMASK BGPPATH CLIST ECLIST IF THEN ELSE CASE TRUE FALSE RT RO UNKNOWN GENERIC GW NET MASK PROTO SOURCE SCOPE CAST DEST IFNAME IFINDEX LEN DEFINED CONTAINS RESET PREPEND FIRST LAST MATCH ROA_CHECK EMPTY WHERE ROLE_MAP NET_PREFIX PREFIX_ROLE

%nonassoc THEN
%nonassoc ELSE

%type <x> term block cmds cmds_int cmd function_body constant constructor print_one print_list var_list var_listn dynamic_attr static_attr function_call symbol bgp_path_expr
%type <rm> role_map role_map_body
%type <f> filter filter_body where_filter
%type <i> type break_command pair_expr ec_kind role_name
...
%token PIPE MODE OPAQUE TRANSPARENT
...
sym_args:
 ...
 | sym_args ROLE_MAP { $$ = $1; $$->type = SYM_ROLE_MAP; }
...
role_map_def:
   ROLE_MAP SYM { $2 = cf_define_symbol($2, SYM_ROLE_MAP, NULL); cf_push_scope( $2 ); }
     role_map_body {
     $2->def = $4;
     $4->name = $2->name;
...
role_map:
   SYM {
     if ($1->class != SYM_ROLE_MAP) cf_error("No such role_map.");
     $$ = $1->def;
   }
 | role_map_body
 ;
...
term:
 ...
 | NET_PREFIX {$$ = f_new_inst(); $$->code = P('n','p'); }
...
bgp_proto:
 ...
 | bgp_proto ROLE role_name ';' { BGP_CFG->role = $3; }
 | bgp_proto ROLE COMPLEX USE role_map ';' { BGP_CFG->role = ROLE_COMP; BGP_CFG->role_map = $5; }
...
conf: ';' | definition | log_config | mrtdump_base | timeformat_base | debug_unix | rtrid | listen | gr_opts | newtab | roa_table | proto | debug_default | filter_def | role_map_def | filter_eval | function_def ;
...
proto: kern_proto '}' | kif_proto '}' | dev_proto '}' | bfd_proto | bgp_proto '}' { bgp_check_config(BGP_CFG); }  | ospf_proto '}' { ospf_proto_finish(); }  | pipe_proto '}' | rip_cfg '}' { RIP_CFG->passwords = get_passwords(); }  | static_proto '}' ;
